...
export const BASE_URLS = {
  ...
  ADMIN_DASHBOARD: process.env.NEXT_PUBLIC_ADMIN_DASHBOARD_URL || 'http://localhost:3002',
} as const;
...
export const API_ENDPOINTS = {
  ...
  STUDENT: {
    ...
    ASSIGNMENT_SUBMIT: (assignmentId: string) => `/api/student/assignments/${assignmentId}/submit`,
    ...
    LEADERBOARD_COURSE: (courseId: string) => `/api/student/leaderboard/courses/${courseId}`,
  ...
  COURSES: {
    ...
    MODULE_BY_ID: (courseId: string, moduleId: string) => `/api/courses/${courseId}/modules/${moduleId}`,
...
// Full URL builders
export const buildUrl = (endpoint: string, baseUrl: string = BASE_URLS.BACKEND): string => {
...
export const getLMSUrl = (): string | null => {
  ...
  const lmsHost = DOMAIN_MAPPINGS.ADMIN_TO_LMS[currentHost as keyof typeof DOMAIN_MAPPINGS.ADMIN_TO_LMS];
