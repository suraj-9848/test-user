...
const MOCK_TESTS: Test[] = [
  ...
  },
  {
    ...
    description: 'Server-side development with Node.js, Express, databases, and API design',
...
const MOCK_QUESTIONS: Question[] = [
  {
    ...
    options: [
      { id: 'a', optionText: '`let` and `const` are block-scoped, `var` is function-scoped', optionOrder: 1 },
      { id: 'b', optionText: 'All three have the same scope behavior', optionOrder: 2 },
      { id: 'c', optionText: '`var` is block-scoped, `let` and `const` are function-scoped', optionOrder: 3 },
  ...
  {
    id: 'q2',
    questionText: 'Which of the following array methods does NOT mutate the original array?',
    questionType: QuestionType.MCQ,
    options: [
  ...
  {
    id: 'q5',
    questionText: 'Which of the following statements about React hooks are correct?',
    questionType: QuestionType.MULTIPLE_SELECT,
    options: [
      { id: 'a', optionText: 'Hooks can only be called at the top level of React functions', optionOrder: 1 },
      { id: 'b', optionText: 'useState returns an array with the current state and a setter function', optionOrder: 2 },
      { id: 'c', optionText: 'useEffect runs after every render by default', optionOrder: 3 },
      { id: 'd', optionText: 'Custom hooks must start with "use"', optionOrder: 4 }
    ],
...
export const mockTestService = {
  ...
  // Get test questions
  getTestQuestions: async (attemptId: string): Promise<{ questions: Question[] }> => {
  ...
  // Save answer
  saveAnswer: async (attemptId: string, questionId: string, answer: any): Promise<{ success: boolean }> => {
  ...
  // Auto-save answers
  autoSave: async (attemptId: string, answers: Record<string, any>): Promise<{ success: boolean }> => {
  ...
  // Submit test
  submitTest: async (attemptId: string): Promise<{ success: boolean; results?: TestResults }> => {
    ...
    const results: TestResults = {
      ...
      grade: mockScore >= 80 ? 'A' : mockScore >= 70 ? 'B' : mockScore >= 60 ? 'C' : 'F',
      passed: mockScore >= 60,
  ...
  logMonitoringEvent: async (event: Omit<MonitoringEvent, 'id' | 'timestamp'>): Promise<{ success: boolean }> => {
  ...
  logMonitoringEventsBatch: async (events: Omit<MonitoringEvent, 'id' | 'timestamp'>[]): Promise<{ success: boolean }> => {
    await new Promise(resolve => setTimeout(resolve, 100));
  ...
  uploadRecording: async (videoBlob: Blob, attemptId: string): Promise<{ success: boolean; url?: string }> => {
  ...
  // Upload video chunk
  uploadRecordingChunk: async (chunkBlob: Blob, attemptId: string, chunkIndex: number): Promise<{ success: boolean }> => {
    await new Promise(resolve => setTimeout(resolve, 500));
    
    console.log(`Mock video chunk upload for attempt ${attemptId}, chunk ${chunkIndex}:`, {
      size: chunkBlob.size,
      type: chunkBlob.type
    });
