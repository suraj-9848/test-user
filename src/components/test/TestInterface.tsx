...
export default function TestInterface({
                ...
                if (testDetails.questions && testDetails.questions.length > 0) {
                    testDetails.questions.forEach((q: Question, index: number) => {
                        console.log(`Question ${index + 1}:`, {
                            id: q.id,
                            type: q.questionType,
                            text: q.questionText ? q.questionText.substring(0, 50) + "..." : "No text"
                        });
                    });
                }
    ...
    const startSecurityMonitoring = () => {
        ...
        const handleFullscreenChange = () => {
            ...
            if (!isFs) {
                // Exited fullscreen: count violation
                logMonitoringEvent(
                    MonitoringEventType.FULLSCREEN_EXIT,
                    SecuritySeverity.HIGH,
                    {}
                ...
                setFullscreenExitCount((prev) => {
                    const count = prev + 1;
                    if (count >= 3) autoSubmitTest("AUTO_VIOLATION");
                    return count;
                });
            }
        };
        ...
        const handleVisibilityChange = () => {
            if (document.hidden) {
                // Update tab switch count functionally to capture latest value
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "tab") {
                ...
                setTabSwitchCount((prevCount) => {
                    ...
                    if (newCount >= 3) autoSubmitTest("AUTO_VIOLATION");
                    return newCount;
                });
                return;
            }
    ...
    const submitTest = async (
        ...
        try {
            ...
            const formattedResponses = answers.map(answer => {
                ...
                console.log("Formatting answer for question:", question, "Answer:", answer);
                
                let formattedAnswer;
                
                // Check if this is a multiple-choice question by looking at the question text
                const isMultipleChoice = question?.questionText?.toLowerCase().includes('select all') || 
                                       question?.questionText?.toLowerCase().includes('multiple') ||
                                       question?.id === 'q2-mcq-multiple'; // Specific check for q2
                
                if (question?.questionType === QuestionType.MCQ) {
                    if (isMultipleChoice) {
                        // For multiple-choice MCQ, send array of selected option IDs
                        formattedAnswer = answer.selectedOptions.length > 0 ? answer.selectedOptions : [];
                        console.log("Multiple-choice MCQ answer formatted:", formattedAnswer);
                    } else {
                        // For single-choice MCQ, send single option ID as string
                        formattedAnswer = answer.selectedOptions[0] || "";
                        console.log("Single-choice MCQ answer formatted:", formattedAnswer);
                    }
                } else {
                    // For text answers, send the text as string
                    formattedAnswer = answer.textAnswer || "";
                    console.log("Text answer formatted:", formattedAnswer);
                }
            ...
            const answeredQuestionIds = formattedResponses.map(r => r.questionId);
        ...
        return (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-900 text-white">
                <div className="bg-gray-800 p-8 rounded-lg text-center max-w-sm">
                    ...
                    <p className="mb-6">
                        This test requires camera access and fullscreen mode.
