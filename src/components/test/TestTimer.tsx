...
export default function TestTimer({ initialTime, onTimeExpired }: TestTimerProps) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setTimeRemaining(prev => {
        ...
        if (newTime <= 300 && newTime > 60) { // 5 minutes
          setIsWarning(true);
          setIsCritical(false);
  ...
  return (
    <div className="flex flex-col items-end space-y-2">
      {/* Timer Display */}
      <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg border ${getBackgroundColor()}`}>
        <Clock className={`h-5 w-5 ${getTimerColor()} ${isCritical ? 'animate-pulse' : ''}`} />
