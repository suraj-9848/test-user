...
const NotificationsPage = () => {
  ...
  const [notifications, setNotifications] = useState([
    ...
    },
    {
      ...
      message: 'Don\'t forget to review your notes for tomorrow\'s Data Structures quiz',
      time: '3 days ago',
      read: true,
  ...
  const filters = [
    ...
    { key: 'discussion', label: 'Discussions', count: notifications.filter(n => n.type === 'discussion').length }
  ];
  ...
  const getColorClasses = (color: string, read: boolean) => {
    ...
    switch (color) {
      case 'blue': return { icon: `text-blue-${opacity}`, bg: `bg-blue-${bgOpacity}` };
      case 'green': return { icon: `text-green-${opacity}`, bg: `bg-green-${bgOpacity}` };
      case 'purple': return { icon: `text-purple-${opacity}`, bg: `bg-purple-${bgOpacity}` };
      case 'orange': return { icon: `text-orange-${opacity}`, bg: `bg-orange-${bgOpacity}` };
      case 'yellow': return { icon: `text-yellow-${opacity}`, bg: `bg-yellow-${bgOpacity}` };
      case 'indigo': return { icon: `text-indigo-${opacity}`, bg: `bg-indigo-${bgOpacity}` };
      default: return { icon: `text-gray-${opacity}`, bg: `bg-gray-${bgOpacity}` };
    }
  };
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Stay updated with your learning progress</p>
            </div>
