...
export const getStudentTests = async (courseId?: string) => {
  const url = courseId ? `/api/student/tests?courseId=${courseId}` : '/api/student/tests';
...
export const submitTestAnswers = async (testId: string, answers: any) => {
  try {
    const response = await apiPost(`/api/student/tests/${testId}/submit`, answers);
    const data = await handleApiResponse(response);
    return data;
  } catch (error) {
...
export const getModuleContent = async (moduleId: string) => {
  try {
    const response = await apiClient(`/api/student/modules/${moduleId}/content`, {
      method: 'GET',
...
export const markModuleComplete = async (moduleId: string) => {
  try {
    ...
    if (error.message.includes('403')) {
      throw new Error('You have not completed all required tasks in this module');
